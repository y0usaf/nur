...
-- Visual/behavioral port of ~/.config/ags/bar-overlay.tsx.
--
...
-- preserving transparent gaps between the blocks like AGS/GTK.
...
local function block(children, opts)
    ...
    return ui.hbox({
        ...
        border = opts.border or 0.7, -- AGS: 0.05em at 14px ≈ 0.7px
...
local function open_edge(edge, content, items, start_x, opts)
    ...
    for _, item in ipairs(items) do
        local name = (opts.name_prefix or "ags-overlay") .. "-" .. edge .. "-" .. item.name
        close_existing(name)
        ...
        -- these tiny AGS-parity module windows directly so clock/date/tray
...
function M.open(opts)
    ...
    close_existing(opts.top_name or "ags-overlay-top")
    close_existing(opts.bottom_name or "ags-overlay-bottom")
